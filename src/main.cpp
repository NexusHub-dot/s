...
namespace {
enum class Mode { Off, Record, Play };
struct Session {
...
void updateStatus() {
    ...
    session.status->setColor(session.mode == Mode::Record ? ccColor3B{255, 115, 110} :
        session.mode == Mode::Play ? ccColor3B{100, 230, 210} : ccColor3B{210, 210, 220});
...
class $modify(PulseGameLayer, GJBaseGameLayer) {
    ...
    void update(float dt) {
        if(static_cast<GJBaseGameLayer*>(this)==session.layer && session.mode!=Mode::Off && session.layer->m_isPaused) {
            session.clock.reset();return;
        }
        if (static_cast<GJBaseGameLayer*>(this) != session.layer || session.mode == Mode::Off ||
            !session.fixedUpdates || session.resetting || session.layer->m_isPaused) {
            if (static_cast<GJBaseGameLayer*>(this) == session.layer) session.clock.reset();
    ...
    void handleButton(bool down, int button, bool isPlayer1) {
        ...
        if (session.mode == Mode::Play) return;
