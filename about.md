Pulse Macro v1.4.2 input-aware checkpoint analyzer.

Frame-window analysis builds hidden analyzer-owned native checkpoints near the actual macro inputs. Each target uses the latest safe restore point before the full ±20-tick search window plus a 12-tick safety margin. Up to 128 checkpoints are retained; dense input clusters share snapshots. Candidate trials restore a nearby checkpoint, replay only the short suffix, shift one input, and still live or die under natural 240 Hz physics.

Checkpoint restores are verified before first use. Unreliable snapshots fall back to an earlier usable checkpoint or the level start instead of aborting the whole run. Version 1.4.2 also includes the v1.4.1 current-checkpoint/full-reset hardening, 0.75-second periodic fallback mode, runtime/ETA display, and nonfatal uncertainty warnings.
