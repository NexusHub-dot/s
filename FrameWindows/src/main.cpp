...
void resetTrial() {
    ...
    if(s.cacheEnabled && !s.search->baseline() && !s.forceFullRestart) {
        int target=s.reference->inputs[s.search->index].tick;
        auto nearest=selectAnalysisCheckpoint(s.analysisCheckpoints,target,maxWindow,checkpointSafetyTicks,true);
        ...
        s.activeCheckpoint=selectAnalysisCheckpoint(s.analysisCheckpoints,target);
...
void captureAnalysisCheckpoint() {
    ...
    if(!s.cacheEnabled || s.search->stage!=Search::Stage::Baseline || s.failedTrial || s.completedTrial ||
        s.physicsTick<=0 || s.physicsTick>=s.reference->endTick || s.analysisCheckpoints.size()>=checkpointCap)return;
    bool due=false;
...
void startAnalysis() {
    ...
    s.cacheSpacing=checkpointSpacing(s.reference->endTick,static_cast<int>(Mod::get()->getSettingValue<int64_t>("fwl-checkpoint-spacing")));
    s.checkpointPlan.clear();s.checkpointPlanCursor=0;
    if(s.cacheEnabled && s.inputAwareCache)s.checkpointPlan=inputAwareCheckpointPlan(*s.reference);
    log::info("Analysis checkpoint cache: {} | strategy {} | planned {} | fallback spacing {}",
        s.cacheEnabled?"on":"off",s.inputAwareCache?"input-aware":"periodic",s.checkpointPlan.size(),s.cacheSpacing);
...
void stopAnalysis(std::string message) {
    ...
    try {
        ...
        report["version"]="v1.4.2";report["level"]=s.reference?s.reference->levelName:"";
        report["cacheEnabled"]=s.cacheEnabled;report["inputAwareCache"]=s.inputAwareCache;
        report["checkpointSpacing"]=s.cacheSpacing;report["plannedCheckpoints"]=static_cast<int64_t>(s.checkpointPlan.size());
    ...
    void processCommands(float dt,bool half,bool last) {
        ...
        if(s.mode==fwl::Mode::Analyzing && s.search && !s.search->finished() && s.reference) {
            if(s.search->stage==fwl::Search::Stage::Baseline && !s.analysisCheckpoints.empty()) {
                auto& cp=s.analysisCheckpoints.back();
                if(tick>=cp.tick && tick<cp.tick+fwl::checkpointValidationTicks)
                    cp.validation.push_back(fwl::pose(s.layer,tick));
            }
            if(s.verifyingCheckpoint) {
                auto& cp=s.analysisCheckpoints[*s.activeCheckpoint];
