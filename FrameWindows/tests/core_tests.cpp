...
int main() {
    ...
    Search bad(1); bad.accept(false); require(bad.stage == Search::Stage::Failed, "failed baseline stops analysis");
    Search changed(1); changed.accept(true); changed.accept(false); changed.accept(false); changed.accept(false);
    require(changed.stage == Search::Stage::Failed, "final baseline rejects changed environment");
