## v1.4.2 input-aware checkpoint planner

The checkpoint cache now targets the macro's actual input timings instead of relying only on periodic snapshots. For each input, Pulse computes the latest safe restore boundary (`input tick - 20f max window - 12 tick safety`). If there are at most 128 unique boundaries, it caches those exact points; if there are more, it samples them across the level timeline so dense click clusters share nearby checkpoints. This should cut most candidate pre-rolls from roughly 0.75–1.0 seconds to only a small fraction of a second on input-dense levels.

The original periodic cache remains available by disabling **Place analysis checkpoints near inputs**. Checkpoint validation/fallback, natural shifted-input physics, conservative unstable-boundary handling, and the final baseline are unchanged.
